...
rule n1:
    input:
        n1_cov = GENCOV_DIR + SPECIES + "{synteny}N1.nodup.nm.{ED}.out"
    output:
        n1_mean = GENCOV_DIR + SPECIES + "{synteny}N1.nodup.nm.{ED}.out.mean",
        n1_1Mbp = RESULTDIR + SPECIES + "{synteny}N1.nodup.nm.{ED}.1Mbp.out",
        n1_100kbp = RESULTDIR + SPECIES + "{synteny}N1.nodup.nm.{ED}.100kbp.out",
        n1_chr = RESULTDIR + SPECIES + "{synteny}N1.nodup.nm.{ED}.chr.out"
    threads: 1
    params:
        hetero = expand("het:{heterogametic}", heterogametic = HETEROGAMETIC),
        homo = expand("homo:{homogametic}", homogametic = HOMOGAMETIC),
    ...
    shell:
        """
        python3 code/n1_genCov.py {input.n1_cov} {params.hetero} {params.homo} > {output.n1_mean}
...
rule calculate_heterozygosity:
    input:
        "{path}" + SPECIES + "{synteny}diffHeterozygosity.bestMatch.small"
    output:
...
rule calculate_allFreq:
    input:
        "{path}" + SPECIES + "{synteny}allFreq.bestMatch.small"
    output:
        Mb = RESULTDIR + SPECIES + "{synteny}allFreq.1Mbp.out",
        kb = RESULTDIR + SPECIES + "{synteny}allFreq.100kbp.out"
    threads: 1
    params:
        chromosomes = CHROMOSOMES
    shell:
...
rule calculate_ratio:
    input:
        "{path}" + SPECIES + "{synteny}gencov.nodup.nm.{ED}.out.norm"
    output:
