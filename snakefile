...
rule proportion_heterozygosity_window:
    ...
    shell:
        ...
        cat {output.het_sorted_window} | sed 's/\t/STARTCOORD/' | sed 's/\t/ENDCOORD/' | awk '{print $1,$2,$3,$4,$4+0.2}' | sed 's/ /\t/g' |  sed 's/\t/STARTCOORD/' | sed 's/\t/ENDCOORD/' | awk ' {{c[$1]++; for (i=2;i<=NF;i++) {{ s[$1"."i]+=$i}}; } END {{for (k in c) {{printf "%s\t", k; for(i=2;i<NF;i++) printf "%.1f\t", s[k"."i]/c[k]; printf "%.1f\n", s[k"."NF]/c[k];}}}}' |  sed 's/STARTCOORD/\t/' | sed 's/ENDCOORD/\t/' > {output.het_sorted_window_mean}
	"""
