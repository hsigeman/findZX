...
rule plotting_chr:
	...
	out_scatter3D_png = protected(RESULTDIR + SPECIES + "{synteny}chr_scatter2D.png"),
        chromosomes = CHROMOSOMES,
	ED = expand("{ED}", ED = EDIT_DIST)
    threads: 1
    shell:
        """
        Rscript code/scatterplot_chr.R {input.cov} {input.snp} {params.out_scatter2D} {params.out_scatter3D} {params.chromosomes} {params.ED} {params.out_scatter2D_png}
        """
